...
function App() {
  ...
  function createWallet() {
    ...
    localStorage.setItem("sendera_wallet", JSON.stringify({
  ...
  function unlockWallet() {
    ...
    try {
      ...
      alert("Unable to unlock wallet. Please import your recovery phrase.");
    }
  }
  ...
  async function refreshTransactions() {
    ...
    try {
      ...
      const response = await fetch(`https://api.etherscan.io/v2/api?chainid=11155111&module=account&action=txlist&address=${wallet.address}&startblock=0&endblock=99999999&page=1&offset=10&sort=desc&apikey=${API_KEY}`);
      const data = await response.json();
      if (data.status === "1") setTransactions(data.result);
      else if (data.result?.length === 0) setTransactions([]);
    } catch (error) {
  ...
  useEffect(() => {
    const savedWallet = localStorage.getItem("sendera_wallet");
    const locked = sessionStorage.getItem("sendera_locked") === "1";
  ...
  if (screen === "locked") {
    return (
      <div style={{ minHeight: "100vh", background: "#020617", color: "white", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", padding: 20, textAlign: "center" }}>
        <div style={{ background: "#0f172a", padding: 28, borderRadius: 20, width: "100%", maxWidth: 380, boxSizing: "border-box" }}>
          <div style={{ fontSize: 48, marginBottom: 10 }}>🔒</div>
          <h2>Wallet Locked</h2>
          <p style={{ color: "#94a3b8", lineHeight: 1.5 }}>
            Your Sendera session is locked. Unlock to continue using this wallet.
          </p>
          <button onClick={unlockWallet} style={{ width: "100%", padding: 15, marginTop: 15, border: "none", borderRadius: 12, background: "#22c55e", color: "white", fontWeight: "bold" }}>
            Unlock Wallet
          </button>
        </div>
      </div>
  ...
  if (screen === "backup") {
    return (
      <div style={{ minHeight: "100vh", background: "#020617", color: "white", padding: 20 }}>
        ...
        <button onClick={() => setScreen("dashboard")} style={{ width: "100%", padding: 16, marginTop: 20 }}>I Saved It</button>
      </div>
    );
  }
  ...
  if (screen === "dashboard") {
    return (
      <div style={{ minHeight: "100vh", background: "#020617", color: "white", padding: 20, paddingBottom: 100 }}>
        ...
        {successTransaction && (
          <TransactionSuccess amount={successTransaction.amount} network={successTransaction.network} address={successTransaction.address} hash={successTransaction.hash} onDone={() => { setSuccessTransaction(null); setActiveTab("home"); }} />
        )}
      </div>
    );
  }

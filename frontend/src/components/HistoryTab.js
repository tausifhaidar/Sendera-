...
function HistoryTab({ transactions = [], selectedNetwork, wallet, onRefresh }) {
  ...
  const networkName = network.name || selectedNetwork;
  ...
  function formatGasFee(tx) {
    try {
      if (tx.gasUsed && tx.gasPrice) return `${Number(ethers.formatEther(BigInt(tx.gasUsed) * BigInt(tx.gasPrice))).toFixed(6)} ${nativeSymbol}`;
      if (resolvedGasFees[tx.hash]) return `${Number(resolvedGasFees[tx.hash]).toFixed(6)} ${nativeSymbol}`;
      return "Loading...";
  ...
  const pill = (background, color) => ({ display: "inline-flex", alignItems: "center", padding: "5px 9px", borderRadius: 999, background, color, fontSize: 11, fontWeight: 800 });

  return (
    <div style={{ maxWidth: 620, margin: "0 auto", paddingBottom: 30 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, marginBottom: 16 }}>
        <div><div style={{ color: "#8d9abb", fontSize: 12 }}>Your activity</div><h2 style={{ margin: "4px 0 0", fontSize: 28 }}>Transaction History</h2><p style={{ color: "#8d9abb", margin: "6px 0 0", fontSize: 12 }}>{networkName}</p></div>
      ...
      {transactions.length > 0 ? transactions.map((tx) => {
        ...
        return (
          <div key={tx.hash} style={{ background: "rgba(13,21,52,.86)", border: "1px solid rgba(91,74,170,.25)", padding: 17, borderRadius: 22, marginTop: 13 }}>
            <button onClick={() => setExpanded(isOpen ? null : tx.hash)} style={{ width: "100%", background: "none", border: "none", color: "white", padding: 0, textAlign: "left", cursor: "pointer" }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}><div style={{ width: 38, height: 38, borderRadius: 12, display: "grid", placeItems: "center", background: isReceived ? "rgba(34,197,94,.14)" : "rgba(124,58,237,.16)", color: isReceived ? "#4ade80" : "#c4b5fd", fontSize: 19 }}>{isReceived ? "↓" : "↑"}</div><div><strong>{isReceived ? "Received" : "Sent"}</strong><div style={{ color: "#7584a2", fontSize: 11, marginTop: 3 }}>{date}</div></div></div>
                <div style={{ textAlign: "right" }}><strong style={{ fontSize: 14 }}>{isReceived ? "+" : "-"}{formatAmount(tx.value)} {nativeSymbol}</strong><div style={{ marginTop: 5 }}>{statusStyle}</div></div>
              </div>
            </button>
            {isOpen && (
              <div style={{ marginTop: 15, paddingTop: 14, borderTop: "1px solid #223257" }}>
                <div style={{ color: "#7e8cab", fontSize: 11 }}>{isReceived ? "From" : "To"}</div><div style={{ marginTop: 5, fontSize: 12, wordBreak: "break-all" }}>{isReceived ? tx.from : tx.to}</div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 13 }}><div style={{ background: "#09132f", padding: 12, borderRadius: 14, border: "1px solid #21345e" }}><div style={{ color: "#7584a2", fontSize: 11 }}>Gas fee</div><div style={{ marginTop: 4, fontWeight: 700 }}>{formatGasFee(tx)}</div></div><div style={{ background: "#09132f", padding: 12, borderRadius: 14, border: "1px solid #21345e" }}><div style={{ color: "#7584a2", fontSize: 11 }}>Status</div><div style={{ marginTop: 4, fontWeight: 700 }}>{status}</div></div></div>
                <div style={{ color: "#7584a2", fontSize: 11, marginTop: 13 }}>Transaction hash</div><div style={{ fontSize: 11, wordBreak: "break-all", marginTop: 5, color: "#c9d3e9" }}>{tx.hash}</div>

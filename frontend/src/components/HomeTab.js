...
function HomeTab({ wallet, balance, selectedNetwork, setSelectedNetwork, setActiveTab }) {
  ...
  return (
    <div style={{ minHeight: "calc(100vh - 20px)", maxWidth: 620, margin: "0 auto", padding: "8px 2px 24px", position: "relative", overflow: "hidden" }}>
      ...
      {showBuy && <div style={{ position: "fixed", inset: 0, zIndex: 90, background: "rgba(1,5,18,.72)", backdropFilter: "blur(10px)", display: "grid", placeItems: "center", padding: 18 }}><div style={{ ...card, width: "100%", maxWidth: 480, padding: 20, background: "linear-gradient(160deg,#161a43,#20112f)" }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><div><div style={{ color: "#9ea9c3", fontSize: 12 }}>Fiat on-ramp</div><h3 style={{ margin: "4px 0 0", fontSize: 24 }}>Buy Crypto</h3></div><button onClick={() => setShowBuy(false)} style={{ border: 0, background: "rgba(255,255,255,.07)", color: "white", width: 36, height: 36, borderRadius: "50%" }}>×</button></div><div style={{ marginTop: 14, padding: 14, borderRadius: 14, background: "rgba(255,255,255,.05)", border: "1px solid rgba(139,92,246,.24)", color: "#c7d0e4", fontSize: 12, lineHeight: 1.5 }}>Buy integrations require a regulated on-ramp provider and production credentials. Sendera does not collect card or bank details itself.</div><button onClick={() => { notify("On-ramp setup required"); setShowBuy(false); setActiveTab?.("settings"); }} style={{ width: "100%", marginTop: 12, padding: 13, border: 0, borderRadius: 13, background: "linear-gradient(135deg,#ff9c2d,#ff6f00)", color: "white", fontWeight: 900 }}>Configure Buy Provider</button></div></div>}
    </div>
  );
}

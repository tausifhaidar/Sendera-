...
const SWAP_CONFIG = {
  ethereumSepolia: { quoter: "0xEd1f6473345F45b75F8179591dd5bA1888cf2FB3", router: "0x3bFA4769FB09eefC5a80d6E87c3B9C650f7Ae48E" },
  baseSepolia: { quoter: "0xC5290058841028F1614F3A6F0F5816cAd0df5E27", router: "0x94cC0AaC535CCDB3C01d6787D6413C739ae12bc4" },
};
const SWAP_POOL_FEE = 3000;
...
function shorten(value, start = 6, end = 4) { return value ? `${value.slice(0, start)}...${value.slice(-end)}` : "—"; }
...
function HomeTab({ wallet, balance, selectedNetwork, setSelectedNetwork, setActiveTab }) {
  ...
  const [swap, setSwap] = useState({ tokenIn: "", tokenOut: "", amount: "" });
  ...
  const card = { background: "rgba(11, 20, 49, 0.72)", border: "1px solid rgba(145, 110, 255, 0.28)", borderRadius: 24, boxShadow: "0 18px 55px rgba(4, 7, 30, .35)", backdropFilter: "blur(18px)", WebkitBackdropFilter: "blur(18px)" };
  ...
  async function discoverWalletTokens() {
    ...
    try {
      const discovered = [];
      if (apiBase) {
        const chainId = { baseSepolia: "84532", ethereumSepolia: "11155111", polygonAmoy: "80002" }[selectedNetwork];
        const response = await fetch(`${apiBase}/api/token-holdings?address=${encodeURIComponent(wallet.address)}&chainid=${chainId}`);
        if (response.ok) {
          const data = await response.json();
          if (Array.isArray(data.holdings)) discovered.push(...data.holdings);
        }
      }
      try {
        const saved = JSON.parse(localStorage.getItem("sendera_tokens") || "[]");
        saved.filter((t) => t.network === selectedNetwork).forEach((t) => {
          if (!discovered.some((item) => item.address?.toLowerCase() === t.address?.toLowerCase())) discovered.push({ ...t, balance: "0", priceUsd: 0 });
        });
      } catch {}
      setWalletTokens(discovered);
      setSwap((s) => ({ ...s, tokenIn: s.tokenIn || discovered[0]?.address || "", tokenOut: s.tokenOut || discovered.find((t) => t.address?.toLowerCase() !== discovered[0]?.address?.toLowerCase())?.address || "" }));
      if (!discovered.length) notify("No ERC-20 holdings found on this network");
    } catch (error) { console.error("Token discovery error", error); notify("Unable to load wallet tokens"); }
    finally { setTokenLoading(false); }
  }

  useEffect(() => { if (showSwap) discoverWalletTokens(); }, [showSwap, selectedNetwork, wallet?.address]);
  useEffect(() => {
    let cancelled = false;
  ...
  const filteredInTokens = walletTokens.filter((token) => `${token.name || ""} ${token.symbol || ""}`.toLowerCase().includes(tokenSearchIn.toLowerCase()));
  const filteredOutTokens = walletTokens.filter((token) => `${token.name || ""} ${token.symbol || ""}`.toLowerCase().includes(tokenSearchOut.toLowerCase()));
  const selectedIn = walletTokens.find((t) => t.address?.toLowerCase() === swap.tokenIn?.toLowerCase());
  const selectedOut = walletTokens.find((t) => t.address?.toLowerCase() === swap.tokenOut?.toLowerCase());

  async function quoteSwap() {
    if (!wallet) return;
    if (!swapSupported) return notify("Swap is unavailable on Polygon Amoy right now");
    if (!selectedIn || !selectedOut) return notify("Select two wallet tokens");
    if (selectedIn.address.toLowerCase() === selectedOut.address.toLowerCase()) return notify("Choose two different tokens");
    if (!swap.amount || Number(swap.amount) <= 0) return notify("Enter a valid amount");
    try {
      setSwapLoading(true); setSwapCostUsd(null);
      const provider = new ethers.JsonRpcProvider(NETWORKS[selectedNetwork].rpc);
      const amountIn = ethers.parseUnits(swap.amount, selectedIn.decimals);
      const quoter = new ethers.Contract(SWAP_CONFIG[selectedNetwork].quoter, QUOTER_ABI, provider);
      const result = await quoter.quoteExactInputSingle.staticCall({ tokenIn: selectedIn.address, tokenOut: selectedOut.address, amountIn, fee: SWAP_POOL_FEE, sqrtPriceLimitX96: 0 });
      setSwapMeta({ inSymbol: selectedIn.symbol, outSymbol: selectedOut.symbol, inDecimals: selectedIn.decimals, outDecimals: selectedOut.decimals });
      setSwapQuote(ethers.formatUnits(result[0], selectedOut.decimals));
      try {
        const feeData = await provider.getFeeData();
        const gasPrice = feeData.gasPrice ?? feeData.maxFeePerGas;
        if (gasPrice && ethUsd) {
          const networkUsd = Number(ethers.formatEther(BigInt(result[3]) * gasPrice)) * ethUsd;
          const inputUsd = Number(selectedIn.priceUsd || 0);
          const serviceUsd = inputUsd > 0 ? Number(swap.amount) * inputUsd * SERVICE_FEE_RATE : 0;
          setSwapCostUsd(networkUsd + serviceUsd);
        }
      } catch {}
      notify("Quote updated");
    } catch (error) { console.error("Swap quote error", error); setSwapQuote(""); notify(error?.shortMessage || "No swap pool/liquidity found for this pair"); }
    finally { setSwapLoading(false); }
  }

  async function executeSwap() {
    if (!swapQuote || !wallet || !selectedIn || !selectedOut) return;
    try {
      setSwapLoading(true);
      const provider = new ethers.JsonRpcProvider(NETWORKS[selectedNetwork].rpc);
      const signer = wallet.connect(provider);
      const amountIn = ethers.parseUnits(swap.amount, selectedIn.decimals);
      const minOut = ethers.parseUnits((Number(swapQuote) * 0.995).toFixed(Math.min(selectedOut.decimals, 8)), selectedOut.decimals);
      const token = new ethers.Contract(selectedIn.address, ERC20_ABI, signer);
      ...
      const tx = await router.exactInputSingle({ tokenIn: selectedIn.address, tokenOut: selectedOut.address, fee: SWAP_POOL_FEE, recipient: wallet.address, amountIn, amountOutMinimum: minOut, sqrtPriceLimitX96: 0 });
      await tx.wait();
      notify(`Swap successful • ${shorten(tx.hash)}`);
      setSwapQuote(""); setSwapCostUsd(null); setSwap({ tokenIn: "", tokenOut: "", amount: "" }); setShowSwap(false); setActiveTab?.("tokens");
    } catch (error) { console.error("Swap execution error", error); notify(error?.shortMessage || "Swap failed"); }
    finally { setSwapLoading(false); }
  }
  ...
  return <div style={{ minHeight: "calc(100vh - 20px)", maxWidth: 560, margin: "0 auto", padding: "8px 2px 24px", position: "relative", overflow: "hidden" }}>
    ...
    {showSwap && <div style={{ position: "fixed", inset: 0, zIndex: 90, background: "rgba(1,5,18,.72)", backdropFilter: "blur(10px)", display: "grid", placeItems: "center", padding: 18 }}><div style={{ ...card, width: "100%", maxWidth: 540, padding: 20, background: "linear-gradient(160deg,#101a43,#180f37)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><div><div style={{ color: "#9ea9c3", fontSize: 12 }}>On-chain swap</div><h3 style={{ margin: "4px 0 0", fontSize: 24 }}>Swap Tokens</h3></div><button onClick={() => { setShowSwap(false); setSwapQuote(""); }} style={{ border: 0, background: "rgba(255,255,255,.07)", color: "white", width: 36, height: 36, borderRadius: "50%" }}>×</button></div>
      {!swapSupported && <div style={{ marginTop: 14, padding: 12, borderRadius: 14, background: "rgba(245,158,11,.10)", border: "1px solid rgba(245,158,11,.25)", color: "#fbbf24", fontSize: 12 }}>Swap is currently available on Base Sepolia and Ethereum Sepolia.</div>}
      <div style={{ marginTop: 14, color: "#91a0bd", fontSize: 11 }}>From token</div><input value={tokenSearchIn} onChange={(e) => setTokenSearchIn(e.target.value)} placeholder="Search wallet tokens..." style={{ ...input, marginTop: 7 }} /><select value={swap.tokenIn} onChange={(e) => setSwap((s) => ({ ...s, tokenIn: e.target.value }))} style={{ ...input, marginTop: 8 }}><option value="">Select token you own</option>{filteredInTokens.map((token) => <option key={token.address} value={token.address}>{token.symbol} — {token.name}</option>)}</select>
      {selectedIn && <div style={{ marginTop: 5, color: "#71809e", fontSize: 10 }}>Balance: {(() => { try { return ethers.formatUnits(selectedIn.balance || "0", selectedIn.decimals); } catch { return "0"; } })()} {selectedIn.symbol}</div>}
      <div style={{ marginTop: 13, color: "#91a0bd", fontSize: 11 }}>To token</div><input value={tokenSearchOut} onChange={(e) => setTokenSearchOut(e.target.value)} placeholder="Search wallet tokens..." style={{ ...input, marginTop: 7 }} /><select value={swap.tokenOut} onChange={(e) => setSwap((s) => ({ ...s, tokenOut: e.target.value }))} style={{ ...input, marginTop: 8 }}><option value="">Select token to receive</option>{filteredOutTokens.map((token) => <option key={token.address} value={token.address}>{token.symbol} — {token.name}</option>)}</select>
      <input value={swap.amount} onChange={(e) => setSwap((s) => ({ ...s, amount: e.target.value }))} placeholder={selectedIn ? `Amount in ${selectedIn.symbol}` : "Amount"} type="number" min="0" step="any" style={{ ...input, marginTop: 10 }} /><div style={{ marginTop: 8, color: "#687795", fontSize: 10 }}>{tokenLoading ? "Discovering wallet tokens..." : `${walletTokens.length} wallet token${walletTokens.length === 1 ? "" : "s"} available`}</div>
      <button disabled={!swapSupported || swapLoading || tokenLoading} onClick={quoteSwap} style={{ width: "100%", padding: 13, marginTop: 10, border: 0, borderRadius: 14, background: "linear-gradient(135deg,#18bfff,#6750ff)", color: "white", fontWeight: 900 }}>{swapLoading ? "Working..." : "Get Quote"}</button>
      {swapQuote && <div style={{ marginTop: 12, padding: 14, borderRadius: 14, background: "rgba(255,255,255,.05)", border: "1px solid rgba(139,92,246,.25)" }}><div style={{ display: "grid", gap: 7, fontSize: 13 }}><div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: "#98a7c4" }}>You Pay</span><strong>{swap.amount} {swapMeta.inSymbol}</strong></div><div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: "#98a7c4" }}>You Receive</span><strong>{Number(swapQuote).toFixed(Math.min(swapMeta.outDecimals, 6))} {swapMeta.outSymbol}</strong></div><div style={{ display: "flex", justifyContent: "space-between", paddingTop: 5, borderTop: "1px solid rgba(120,140,190,.14)" }}><span style={{ color: "#98a7c4" }}>Network cost</span><strong>{swapCostUsd != null ? `≈ $${swapCostUsd.toFixed(2)}` : "Calculating..."}</strong></div></div><div style={{ marginTop: 7, color: "#70809f", fontSize: 10 }}>Includes estimated network gas + Sendera service cost.</div><div style={{ marginTop: 6, color: "#8b97af", fontSize: 11 }}>~0.5% slippage protection • ERC-20 only</div><button disabled={swapLoading} onClick={executeSwap} style={{ width: "100%", marginTop: 10, padding: 13, border: 0, borderRadius: 13, background: "#22c55e", color: "#04120a", fontWeight: 900 }}>{swapLoading ? "Swapping..." : "Confirm Swap"}</button></div>}
    </div></div>}

    {showBuy && <div style={{ position: "fixed", inset: 0, zIndex: 90, background: "rgba(1,5,18,.72)", backdropFilter: "blur(10px)", display: "grid", placeItems: "center", padding: 18 }}><div style={{ ...card, width: "100%", maxWidth: 480, padding: 20, background: "linear-gradient(160deg,#161a43,#20112f)" }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><div><div style={{ color: "#9ea9c3", fontSize: 12 }}>Fiat on-ramp</div><h3 style={{ margin: "4px 0 0", fontSize: 24 }}>Buy Crypto</h3></div><button onClick={() => setShowBuy(false)} style={{ border: 0, background: "rgba(255,255,255,.07)", color: "white", width: 36, height: 36, borderRadius: "50%" }}>×</button></div><div style={{ marginTop: 14, padding: 14, borderRadius: 14, background: "rgba(255,255,255,.05)", border: "1px solid rgba(139,92,246,.24)", color: "#c7d0e4", fontSize: 12, lineHeight: 1.5 }}>Buy integrations require a regulated on-ramp provider and its production application key. Sendera does not collect card or bank details itself.</div><button onClick={() => { notify("On-ramp setup required before purchase"); setShowBuy(false); setActiveTab?.("settings"); }} style={{ width: "100%", marginTop: 12, padding: 13, border: 0, borderRadius: 13, background: "linear-gradient(135deg,#ff9c2d,#ff6f00)", color: "white", fontWeight: 900 }}>Configure Buy Provider</button></div></div>}
  </div>;
}

    ...
    <div style={{ minHeight: "calc(100vh - 20px)", maxWidth: 560, margin: "0 auto", padding: "8px 2px 24px", position: "relative", overflow: "hidden" }}>
      ...
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 6px 14px" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => setActiveTab?.("receive")} style={{ ...card, width: 42, height: 42, color: "white", cursor: "pointer" }}>⌗</button>
          <button onClick={() => setActiveTab?.("settings")} style={{ ...card, width: 42, height: 42, color: "white", cursor: "pointer" }}>⚙</button>
        </div>
      </header>

      <div style={{ margin: "0 0 12px 2px" }}>
        <select value={selectedNetwork} onChange={(e) => { setSelectedNetwork?.(e.target.value); notify(`Network: ${NETWORKS[e.target.value]?.name}`); }} style={{ border: "1px solid rgba(130,150,255,.30)", background: "linear-gradient(135deg, rgba(24,39,86,.92), rgba(31,18,71,.90))", color: "#eef2ff", borderRadius: 999, padding: "10px 14px", fontWeight: 800, fontSize: 12, cursor: "pointer", outline: "none" }}>
          {networks.map((network) => <option key={network.key} value={network.key} style={{ background: "#0b1227", color: "white" }}>● {network.name}</option>)}
        </select>
      </div>
      ...
      <section style={{ ...card, padding: 22, position: "relative", overflow: "hidden", background: "linear-gradient(135deg, rgba(22,27,74,.92), rgba(20,10,59,.74) 55%, rgba(7,36,73,.80))" }}>
        ...
        <div style={{ marginTop: 5, fontSize: 40, lineHeight: 1, fontWeight: 900, letterSpacing: -1.7, whiteSpace: "nowrap" }}>{Number(balance || 0).toFixed(4)} ETH</div>
        <div style={{ marginTop: 8, color: "#92a0bd", fontSize: 13 }}>{networkName}</div>
        <div style={{ marginTop: 8, color: "#d4dcf0", fontSize: 15, fontWeight: 800 }}>{portfolioUsd}</div>
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
        {action("Send", "Crypto", "↑", "linear-gradient(145deg,#7b31ff,#5b20ff)", () => setActiveTab?.("send"))}
        {action("Receive", "Crypto", "↓", "linear-gradient(145deg,#22e77c,#08b862)", () => setActiveTab?.("receive"))}
        {action("Swap", "Tokens", "⇄", "linear-gradient(145deg,#19b9ff,#0677d8)", () => setShowSwap(true))}
        {action("Buy", "Crypto", "+", "linear-gradient(145deg,#ff9c2d,#ff6f00)", () => setShowBuy(true))}
      </div>

      <section style={{ ...card, marginTop: 14, overflow: "hidden" }}>
        <div style={{ display: "flex", justifyContent: "space-between", padding: "17px 17px 13px", borderBottom: "1px solid rgba(120,140,190,.13)" }}><div style={{ fontSize: 19, fontWeight: 850 }}>Portfolio</div><div style={{ color: "#7d8ca9", fontSize: 11 }}>{ethUsd ? "Live ETH price" : "Price feed offline"}</div></div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "16px 17px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}><div style={{ width: 44, height: 44, borderRadius: "50%", display: "grid", placeItems: "center", background: "linear-gradient(145deg,#4d73ff,#2637b7)", fontSize: 24 }}>Ξ</div><div><div style={{ fontWeight: 800, fontSize: 15 }}>Ethereum</div><div style={{ color: "#8f9bb3", marginTop: 3, fontSize: 12 }}>ETH • {Number(balance || 0).toFixed(4)}</div></div></div>
          <div style={{ textAlign: "right" }}><div style={{ fontWeight: 800, fontSize: 14 }}>{portfolioUsd}</div><div style={{ color: "#8994ab", fontSize: 11, marginTop: 3 }}>{ethUsd ? `$${ethUsd.toLocaleString()}/ETH` : "—"}</div></div>
        </div>
        <button onClick={() => setActiveTab?.("tokens")} style={{ width: "100%", border: 0, borderTop: "1px solid rgba(120,140,190,.13)", background: "rgba(255,255,255,.02)", color: "#7fc4ff", padding: 14, textAlign: "left", fontWeight: 800, cursor: "pointer" }}>Manage tokens →</button>
      </section>

      <section style={{ ...card, marginTop: 14, padding: 16 }}>
        <div style={{ fontSize: 18, fontWeight: 850 }}>Security</div>
        <div style={{ marginTop: 6, color: "#8c99b2", fontSize: 12 }}>Wallet encrypted locally. Auto-lock is enabled.</div>
        <div style={{ display: "flex", gap: 9, marginTop: 12 }}><button onClick={() => setActiveTab?.("settings")} style={{ flex: 1, padding: 12, borderRadius: 13, border: "1px solid #293b68", background: "#101c39", color: "white", fontWeight: 800 }}>Security Settings</button><button onClick={() => setActiveTab?.("history")} style={{ flex: 1, padding: 12, borderRadius: 13, border: "1px solid rgba(139,92,246,.35)", background: "linear-gradient(135deg,#31206f,#203e78)", color: "white", fontWeight: 800 }}>Activity</button></div>
      </section>
      ...
      {showSwap && (
        <div style={{ position: "fixed", inset: 0, zIndex: 90, background: "rgba(1,5,18,.72)", backdropFilter: "blur(10px)", display: "grid", placeItems: "center", padding: 18 }}>
          <div style={{ ...card, width: "100%", maxWidth: 520, padding: 20, background: "linear-gradient(160deg,#101a43,#180f37)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><div><div style={{ color: "#9ea9c3", fontSize: 12 }}>On-chain swap</div><h3 style={{ margin: "4px 0 0", fontSize: 24 }}>Swap Tokens</h3></div><button onClick={() => setShowSwap(false)} style={{ border: 0, background: "rgba(255,255,255,.07)", color: "white", width: 36, height: 36, borderRadius: "50%" }}>×</button></div>
            ...
            {swapQuote && <div style={{ marginTop: 12, padding: 14, borderRadius: 14, background: "rgba(255,255,255,.05)", border: "1px solid rgba(139,92,246,.25)" }}><div style={{ color: "#98a7c4", fontSize: 11 }}>Estimated receive</div><div style={{ marginTop: 4, fontSize: 22, fontWeight: 900 }}>{Number(swapQuote).toFixed(Math.min(swapMeta.outDecimals, 6))} {swapMeta.outSymbol}</div><div style={{ marginTop: 5, color: "#8b97af", fontSize: 11 }}>~0.5% slippage protection • ERC-20 only</div><button disabled={swapLoading} onClick={executeSwap} style={{ width: "100%", marginTop: 10, padding: 13, border: 0, borderRadius: 13, background: "#22c55e", color: "#04120a", fontWeight: 900 }}>{swapLoading ? "Swapping..." : "Confirm Swap"}</button></div>}
          </div>
        </div>
      )}
      ...
      {showBuy && (
        <div style={{ position: "fixed", inset: 0, zIndex: 90, background: "rgba(1,5,18,.72)", backdropFilter: "blur(10px)", display: "grid", placeItems: "center", padding: 18 }}>
          <div style={{ ...card, width: "100%", maxWidth: 480, padding: 20, background: "linear-gradient(160deg,#161a43,#20112f)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><div><div style={{ color: "#9ea9c3", fontSize: 12 }}>Fiat on-ramp</div><h3 style={{ margin: "4px 0 0", fontSize: 24 }}>Buy Crypto</h3></div><button onClick={() => setShowBuy(false)} style={{ border: 0, background: "rgba(255,255,255,.07)", color: "white", width: 36, height: 36, borderRadius: "50%" }}>×</button></div>
            <div style={{ marginTop: 14, padding: 14, borderRadius: 14, background: "rgba(255,255,255,.05)", border: "1px solid rgba(139,92,246,.24)", color: "#c7d0e4", fontSize: 12, lineHeight: 1.5 }}>Buy integrations require a regulated on-ramp provider and its production application key. Sendera does not collect card or bank details itself.</div>
            <button onClick={() => { notify("On-ramp setup required before purchase"); setShowBuy(false); setActiveTab?.("settings"); }} style={{ width: "100%", marginTop: 12, padding: 13, border: 0, borderRadius: 13, background: "linear-gradient(135deg,#ff9c2d,#ff6f00)", color: "white", fontWeight: 900 }}>Configure Buy Provider</button>
          </div>
        </div>
      )}
    </div>

...
function TokenTab({ wallet, selectedNetwork }) {
  ...
  async function addToken() {
    alert("Manual token import is no longer required for wallet holdings. Receive the token on this network and Sendera will discover it automatically.");
  }
  ...
  useEffect(() => {
    ...
    async function loadNative() {
      try {
        const price = await fetchNativeUsdPrice(selectedNetwork);
        if (!cancelled) setNativePrice(price);
  ...
  const enriched = merged.map((token) => {
    const price = tokenPrices[token.address?.toLowerCase()] ?? Number(token.priceUsd || 0) || 0;
    return { ...token, livePriceUsd: price };
  });
  ...
  return (
    <div style={{ maxWidth: 620, margin: "0 auto", paddingBottom: 30 }}>
      <div style={{ marginBottom: 18 }}>
        <div style={{ color: "#8d9abb", fontSize: 12 }}>Assets & live prices</div>
        <h2 style={{ margin: "4px 0 0", fontSize: 28 }}>Portfolio</h2>
        <p style={{ color: "#8d9abb", margin: "6px 0 0", fontSize: 12 }}>{network.name || selectedNetwork}</p>
      </div>
      ...
      <section style={{ ...card, background: "linear-gradient(145deg,rgba(28,26,75,.94),rgba(8,17,45,.98))" }}>
        ...
        <div style={{ display: "flex", gap: 12, marginTop: 10, flexWrap: "wrap", fontSize: 11, color: "#9aa8c4" }}>
          <span>{nativeBalance} {network.symbol || "NATIVE"} · {nativePrice ? formatUsd(nativePrice) : "—"}</span>
          <span>{priceLoading ? "Updating prices…" : "Live prices"}</span>
        </div>
      ...
      {enriched.length === 0 ? (
        <div style={{ ...card, marginTop: 16, color: "#8b98b5", textAlign: "center" }}>
          <div style={{ color: "#dce4f2", fontWeight: 700 }}>No token balances found</div>
          <div style={{ fontSize: 12, marginTop: 5 }}>Token holdings on this EVM network are discovered automatically.</div>
        </div>
      ) : enriched.map((token) => {
        const key = `${selectedNetwork}:${token.address}`;
        ...
        return (
          <div key={key} style={{ ...card, marginTop: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center" }}>
              <div>
                <strong>{token.name || token.symbol}</strong>
                <div style={{ color: "#7c8aa6", fontSize: 12, marginTop: 3 }}>{token.symbol}</div>
              </div>
              <div style={{ textAlign: "right" }}>
                <strong>{Number(balanceText).toFixed(6)}</strong>
                <div style={{ color: "#7c8aa6", fontSize: 11, marginTop: 3 }}>{token.livePriceUsd ? `${formatUsd(usd)} · ${formatUsd(token.livePriceUsd)}` : "Price unavailable"}</div>
              </div>
            </div>
            <div style={{ fontSize: 10, color: "#697896", wordBreak: "break-all", marginTop: 11 }}>{token.address}</div>
            <input value={form.to} onChange={(e) => setSendForm((p) => ({ ...p, [key]: { ...form, to: e.target.value } }))} placeholder="Recipient 0x..." style={{ width: "100%", padding: 12, marginTop: 12, borderRadius: 13, border: "1px solid #293b68", background: "#09132f", color: "white", boxSizing: "border-box" }} />
            <input value={form.amount} onChange={(e) => setSendForm((p) => ({ ...p, [key]: { ...form, amount: e.target.value } }))} placeholder={`Amount ${token.symbol || "token"}`} type="number" style={{ width: "100%", padding: 12, marginTop: 8, borderRadius: 13, border: "1px solid #293b68", background: "#09132f", color: "white", boxSizing: "border-box" }} />
            <button onClick={() => sendToken(token)} disabled={sending === key} style={{ width: "100%", padding: 13, marginTop: 9, border: 0, borderRadius: 13, background: "linear-gradient(135deg,#2563eb,#0ea5e9)", color: "white", fontWeight: 800 }}>{sending === key ? "Sending..." : `Send ${token.symbol || "Token"}`}</button>
          </div>
        );
      })}
    </div>
